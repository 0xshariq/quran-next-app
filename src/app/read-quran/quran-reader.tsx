...
export default function QuranReader() {
  ...
  return (
    <QuranReaderContent
      initialSurah={surahParam ?? null}
      initialPara={paraParam ?? null}
      initialPage={pageParam ?? null}
    />
  );
}
...
function QuranReaderContent({
  ...
  const handleNext = () => {
    const maxPages =
      viewMode === "surah" ? selectedSurah.totalPages : selectedPara.totalPages;
  ...
  const handlePageInputBlur = () => {
    ...
    if (isNaN(pageNumber) || pageNumber < 1 || pageNumber > maxPages) {
      toast.error(`Please enter a number between 1 and ${maxPages}.`);
          ...
          <CardContent className="p-6 space-y-6">
            <div className="flex justify-between items-center flex-wrap gap-4">
              <div className="flex items-center space-x-4 flex-wrap gap-4">
                ...
                <PageInput
                  ...
                  maxPages={
                    viewMode === "surah"
                      ? selectedSurah.totalPages
                      : selectedPara.totalPages
                  }
                />
              </div>
              <PageNavigation
            ...
            >
              <Image
                src={imagePath}
                alt={`${
                  viewMode === "surah" ? selectedSurah.name : selectedPara.name
                } - Page ${currentPage}`}
                fill
...
function ViewModeSelect({
  ...
  viewMode: "surah" | "para";
  setViewMode: (mode: "surah" | "para") => void;
}) {
  return (
    <div>
      <Label htmlFor="view-mode" className="text-amber-800 dark:text-amber-200">
        View Mode
      </Label>
      <Select
        value={viewMode}
      ...
      >
        <SelectTrigger
          id="view-mode"
          className="w-[180px] border-amber-300 dark:border-slate-600"
        >
...
function ParaSelect({
  ...
  selectedPara: Para;
  setSelectedPara: (para: Para) => void;
}) {
  return (
    <div>
      <Label
